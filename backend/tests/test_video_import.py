...
from backend.app.domain.errors import DuplicateVideoError
...
def small_video_bytes(tmp_path: Path, fps: float = 12.0, frames: int = 6) -> bytes:
...
def test_import_probes_video_and_builds_canonical_mapping(tmp_path: Path) -> None:
    ...
    assert imported["canonical_frame_count"] == 12
    ...
    assert mapping[-1]["original_frame"] == 5
