...
→ automatic source-linked 3-class segment annotation and human review
...
13. Source provenance for automatically materialized ground-truth segments.
...
tracking:
  tracker: botsort
  tracker_config_version: botsort-dedicated-reid-v3
...
- When processing completes for an initially unlabeled video, the selected
Threshold or AI suggestions become editable ground-truth segments, and
uncovered detected-track ranges become `others` segments.
...
Threshold-based suggestions are automatically materialized as editable final
segments for an initially unlabeled video. They retain threshold provenance and
are never allowed to overwrite later human edits.
...
16. Initial processing materializes selected suggestions only when no human
    annotations exist; later suggestions never overwrite human annotations.
